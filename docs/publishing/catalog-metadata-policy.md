...
公開済み書籍のProfile必須module不足は、`docs/publishing/ux-profiles.md`を定義元として`debt.missingRequiredUxModules`へ集計する。計画書籍は公開実体が成立するまでこのgateの対象外とする。実ファイル監査済みの既存欠損は`docs/publishing/ux-required-module-debt-baseline.json`に未実装理由、根拠Issue、追跡Issueを記録し、debtとして可視化したまま許容する。baselineにない新規欠損と、module実装後に残るstale baselineはCI failureとする。baselineを変更する場合は、新規欠損を黙って受け入れる再生成ではなく、理由と追跡Issueを伴うレビュー対象の変更として扱う。
