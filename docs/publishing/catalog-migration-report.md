...
`BioinformaticsGuide-book` は管理リポジトリがPrivateである一方、公開Pagesでは正本全文を提供する。この組み合わせは `repoVisibility: "private"` と `publicationScope: "full-public"` で表し、読者向けカタログではリポジトリリンクを非表示にしたまま全文公開であることを案内する。
...
| 解決済み | UX profile/modules | #193と#205で実書籍の証跡を確認し、実体に合わせて`ux.profile` / `ux.modules`を更新した。確認済みの必須module不足は残る。 | #197で必須module不足を決定的debtとして構造化し、既存欠損を許容しながら新規欠損とstale baselineを拒否するgateを追加した。 |
...
- 対象Issue: [#195](https://github.com/itdojp/it-engineer-knowledge-architecture/issues/195)
- 実装PR: [#203](https://github.com/itdojp/it-engineer-knowledge-architecture/pull/203)
- merge commit: `bbd9e1e1e8c2327321f72b45eef51befb8939d2f`
- [PR CI](https://github.com/itdojp/it-engineer-knowledge-architecture/actions/runs/29170008482)、[main validation](https://github.com/itdojp/it-engineer-knowledge-architecture/actions/runs/29170166711)、[Deploy Pages / production smoke](https://github.com/itdojp/it-engineer-knowledge-architecture/actions/runs/29170166708)、[Pages drift](https://github.com/itdojp/it-engineer-knowledge-architecture/actions/runs/29170220923)はいずれも成功した。
- PRレビュー1件を修正し、再レビュー後の未解決thread 0を確認した。merge直後の確認時点で、本番`/books/`は49冊、旧識別子非表示、`build-info.json`のSHA一致を確認した。
